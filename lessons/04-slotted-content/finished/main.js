...
class NMACard extends HTMLElement {
  constructor() {
    ...
    const headingSlot = this.createSlot("heading");
    const contentSlot = this.createSlot("content");

    this.shadowRoot.append(headingSlot, contentSlot);
  }
...
class NMAColorGroup extends HTMLElement {
  constructor() {
    ...
    this.toggleColor = this.getAttribute("toggleColor");

    const slot = document.createElement("slot");
    this.toggleButton = document.createElement("button");
    this.toggleButton.innerHTML = `Click me to turn the boxes ${this.toggleColor}`;


    this.shadowRoot.append(this.toggleButton, slot);
    this.handleToggle();
  }
  ...
  handleToggle() {
    this.toggleButton.addEventListener("click", () => {
      this.colorElements.forEach((colorElement) => {
        colorElement.setAttribute("color", this.toggleColor);
      });
    });
  }

  get colorElements() {
    return this.querySelectorAll("nma-color");
  }
}
